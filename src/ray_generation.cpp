...
{
    frame_buffer.resize(static_cast<float>(width) * static_cast<float>(height));
}
...
Payload RayGenerationApp::Miss(const Ray& ray) const
{
	...
	payload.color = { 0.0f, 0.2f, 0.7f + 0.3f * t }; // {ray.direction.x, ray.direction.y, 0};
    return payload;
}
...
Ray Camera::GetCameraRay(short x, short y) const
{
    ...
    float u = (2.0f * (static_cast<float>(x) + 0.5f) / static_cast<float>(width) - 1.0f) * aspect_ratio;
...
{
    return Ray(float3{0, 0, 0}, float3{0, 0, 0});
}
